...
colors = ((0, 0, 225), (0, 225, 0), (225, 0, 0), (244, 164, 96))  # синий, зеленый, красный, бежевый
...
def pauseScreen():
    pause = pg.Surface((800, 500), pg.SRCALPHA)
...
def main():
    ...
    pg.display.set_caption('Тетрис by Maryanin Daniil')
    showText('Тетрис')
    while True:
        runTetris()
        pauseScreen()
...
def gamecup(cup):
    ...
    # фон игрового поля
    pg.draw.rect(display_surf, (0, 191, 255), (side_margin, top_margin, block * cup_w, block * cup_h))
    for x in range(cup_w):
...
def drawTitle():
    ...
    titleRect.topleft = (window_w - 380, 20)
    display_surf.blit(titleSurf, titleRect)
...
def drawnextFig(fig):
    nextSurf = basic_font.render('Следующая фигура:', True, txt_color)
    nextRect = nextSurf.get_rect()
    nextRect.topleft = (window_w - 175, 200)
    display_surf.blit(nextSurf, nextRect)
    drawFig(fig, pixelx=window_w - 140, pixely=230)
